...
- **直播间号**：仅支持纯数字输入。
- **并发数量**：可设置范围 `10` 到 `10000`。并发数越高，对网络和电脑性能的占用也越大，请根据自身设备情况合理设置。
...
可能是 B 站登录状态过期，或受到账号/网络风控影响。删除失效账号后重新登录即可恢复。

### 没有显示掉宝内容

请确认直播间号无误、账号已正常登录，并稍等片刻，让应用完成账号和直播间的检查。掉宝内容由 B 站实时状态决定，应用不会人为生成不存在的掉宝任务。
